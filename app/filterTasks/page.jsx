...
import { getEvents, getMembers } from "../fetch";
import styles from "../components/ViewCalendarTask/ViewCalendarTask.module.css"
...
const Page = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      const eventsTask = await getEvents();
          ...
          <div>
            {
              events.map((item, i) => {
                ...
                const assignedAvatars = avatar.filter(user =>
                  item?.assignedTo?.avatar || item?.asigned?.includes(user.id)
                );
